...
; HTML split into small segments to prevent MASM A2041 string length limit errors
szHtmlStart        db "<html><head><title>RIS Worklist Manager</title>"
                   db "<style>"
                   db "table { border-collapse: collapse; width: 100%; font-size: 14px; box-shadow: 0 0 20px rgba(0,0,0,0.15); }"
                   db "th, td { border: 1px solid #dddddd; padding: 12px 15px; }"
                   db "th { background-color: #009879; color: #ffffff; text-align: left; }"
                   db "tr { border-bottom: 1px solid #dddddd; }"
                   db "tr:nth-child(even) { background-color: #f3f3f3; }"
                   db "input[type='text'] { margin-bottom: 10px; padding: 5px; width: 180px; margin-right: 10px; }"
                   db "</style>"
                   db "<script>"
                   db "function f(r){var c=r.cells;"
                   db "var n=['PatientName','PatientID','BirthDate','Sex','ReqPhys','ReqSvc','ProcDesc',"
                   db "'Reason','Accession','ProcID','Priority','StationAE','StartDate','StartTime',"
                   db "'PerfPhys','SPSDesc','SPSID','StationName','Location','Status','Modality',"
                   db "'RefPhys','StudyDesc'];"
                   db "for(var i=0;i<c.length&&i<n.length;i++){"
                   db "var e=document.getElementsByName(n[i])[0];"
                   db "if(e)e.value=c[i].innerText||c[i].textContent;}}"
                   db "</script>"
                   db "</head><body style='font-family: Arial, sans-serif; padding: 20px; color: #333;'>",0
...
szHtmlForms        db "<h3>Upload patients.csv</h3>"
                   ...
                   db "<form action='/manual' method='POST'>"
                   db "<input type='text' name='PatientName' placeholder='Patient Name'> "
                   db "<input type='text' name='PatientID' placeholder='Patient ID'> "
                   db "<input type='text' name='BirthDate' placeholder='Birth Date'> "
                   db "<input type='text' name='Sex' placeholder='Sex'><br>"
                   ...
                   db "<input type='text' name='Priority' placeholder='Priority'> "
                   db "<input type='text' name='StationAE' placeholder='Station AE'><br>"
                   db "<input type='text' name='StartDate' placeholder='Start Date'> "
                   db "<input type='text' name='StartTime' placeholder='Start Time'> "
                   ...
                   db "<input type='text' name='Status' placeholder='Status'><br>"
                   ...
                   db "<input type='submit' value='Add/Update Patient'></form><hr><h3>Current Database</h3>",0
                   
szHtmlEnd          db "</body></html>",0

; Manual HTTP field map parsing tokens
szF0 db "PatientName=",0
szF1 db "PatientID=",0
szF2 db "BirthDate=",0
szF3 db "Sex=",0
szF4 db "ReqPhys=",0
...
szF9 db "ProcID=",0
szF10 db "Priority=",0
szF11 db "StationAE=",0
szF12 db "StartDate=",0
szF13 db "StartTime=",0
szF14 db "PerfPhys=",0
szF15 db "SPSDesc=",0
szF16 db "SPSID=",0
szF17 db "StationName=",0
szF18 db "Location=",0
szF19 db "Status=",0
szF20 db "Modality=",0
szF21 db "RefPhys=",0
szF22 db "StudyDesc=",0

g_FieldNames dd OFFSET szF0, OFFSET szF1, OFFSET szF2, OFFSET szF3, OFFSET szF4
             dd OFFSET szF5, OFFSET szF6, OFFSET szF7, OFFSET szF8, OFFSET szF9
             dd OFFSET szF10, OFFSET szF11, OFFSET szF12, OFFSET szF13, OFFSET szF14
             dd OFFSET szF15, OFFSET szF16, OFFSET szF17, OFFSET szF18, OFFSET szF19
             dd OFFSET szF20, OFFSET szF21, OFFSET szF22
...
ValidateEntry PROC pEntry:DWORD
    ; Accession Number (Field 8) is the ONLY mandatory field
    invoke GetFieldPtr, pEntry, 8
    cmp BYTE PTR [eax], 0
    je VE_No

    ; Optional format validations (Only checked if context data is provided)
...
ServeHttpUI:
    ...
    invoke SendText, sock, CStr("<table>")
    mov isHeader, 1
CSVLoop:
    invoke crt_fgets, OFFSET g_fileLine, LINE_SIZE, hFile
    cmp eax, 0
...
ManualFieldLoop:
    ...
    mov eax, fieldIdx
    shl eax, 2
    add eax, OFFSET g_FieldNames
    mov edx, DWORD PTR [eax]
    
    invoke crt_strstr, pBody, edx
    test eax, eax
    jz NextManualField
    ...
    ; FIX: GetFieldPtr modifies ecx natively. We must push/pop 
    ; the string pointer safely across the procedure call.
    push ecx 
    invoke GetFieldPtr, ADDR entry, fieldIdx
    mov edi, eax
    pop esi 
...
NotPlus:
    ...
    cmp ax, 3032h  ; "%20" Little Endian check for spaces
    jne CheckPercent2C
    mov bl, ' '
    add esi, 2
    jmp NotPercent
CheckPercent2C:
    cmp ax, 4332h  ; "%2C" Check for commas
    je IsComma
    cmp ax, 6332h  ; "%2c" Check for commas lower case
    jne NotPercent
IsComma:
    mov bl, ','
    add esi, 2
...
UploadLineLoop:
    ...
    cmp al, 0
    je PostRedirect
    cmp al, '-' ; Reached webkit multipart tail border metadata boundaries
    je PostRedirect
    cmp al, 13
    je SkipEmptyChar
    cmp al, 10
...
ARB_Loop:
    ...
    ; If HTTP, we buffer until \r\n\r\n
    cmp isHttp, 1
    je ARB_BufferAll
    
    ; Telnet Line Processing
    cmp al, 0Dh
    je ARB_NextInc
    cmp al, 0Ah
    je ARB_LineComplete
...
PC_CheckTimers:
    invoke GetClientTypePtr, clientIdx
    cmp DWORD PTR [eax], 1
    je PC_Done ; Don't timeout HTTP clients aggressively like Telnet
...
AC_Add:
    ...
    ; Set client type (0 = Telnet, 1 = HTTP)
