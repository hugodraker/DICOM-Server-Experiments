...
#define ID_TRAY_SETTINGS    1002  /* NEW: Settings Tray Menu Item */
#define ID_TRAY_EXIT        1003

/* Settings Window Control IDs */
#define ID_EDIT_AET         2001
#define ID_EDIT_TELNET      2002
#define ID_EDIT_HTTP        2003
#define ID_EDIT_DICOM       2004
#define ID_EDIT_TIMEOUT     2005
#define ID_EDIT_DEBUG       2006
#define ID_BTN_SAVE         2007
#define ID_BTN_TOGGLE       2008
...
"<button type='button' onclick='location.href=\"/\"' class='top-btn bg-green'>New Patient</button>"
...
static void write_log(const char *msg) {
    ...
    /* Print to console and flush immediately */
...
static void ServerStart(void) {
    ...
    /* Update menu text and checkmark */
    ModifyMenu(g_hMenu, ID_TRAY_TOGGLE, MF_BYCOMMAND | MF_STRING | MF_CHECKED, ID_TRAY_TOGGLE, szMenuStop);
    CheckMenuItem(g_hMenu, ID_TRAY_TOGGLE, MF_BYCOMMAND | MF_CHECKED);
    
    /* Repaint settings toggle button if open */
    if (g_hSettingsWnd && IsWindow(g_hSettingsWnd)) {
        InvalidateRect(GetDlgItem(g_hSettingsWnd, ID_BTN_TOGGLE), NULL, TRUE);
    }
    write_log(szServerStarted);
}
...
static void ServerStop(void) {
    ...
    /* Update menu text and remove checkmark */
    ModifyMenu(g_hMenu, ID_TRAY_TOGGLE, MF_BYCOMMAND | MF_STRING | MF_UNCHECKED, ID_TRAY_TOGGLE, szMenuStart);
    CheckMenuItem(g_hMenu, ID_TRAY_TOGGLE, MF_BYCOMMAND | MF_UNCHECKED);
    
    /* Repaint settings toggle button if open */
    if (g_hSettingsWnd && IsWindow(g_hSettingsWnd)) {
        InvalidateRect(GetDlgItem(g_hSettingsWnd, ID_BTN_TOGGLE), NULL, TRUE);
    }
    write_log(szServerStopped);
}
...
static LRESULT CALLBACK SettingsWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
        case WM_CREATE: {
            int y = 15;
            int lblW = 110, editW = 210, h = 22, spacing = 30;
            char buf[64];

            CreateWindow("STATIC", "AE Title:", WS_CHILD | WS_VISIBLE, 20, y+2, lblW, h, hWnd, NULL, g_hInstance, NULL);
            CreateWindow("EDIT", g_aeCalled, WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL, 135, y, editW, h, hWnd, (HMENU)ID_EDIT_AET, g_hInstance, NULL);
            y += spacing;

            snprintf(buf, sizeof(buf), "%d", g_TelnetPort);
            CreateWindow("STATIC", "Telnet Port:", WS_CHILD | WS_VISIBLE, 20, y+2, lblW, h, hWnd, NULL, g_hInstance, NULL);
            CreateWindow("EDIT", buf, WS_CHILD | WS_VISIBLE | WS_BORDER | ES_NUMBER, 135, y, editW, h, hWnd, (HMENU)ID_EDIT_TELNET, g_hInstance, NULL);
            y += spacing;

            snprintf(buf, sizeof(buf), "%d", g_HttpPort);
            CreateWindow("STATIC", "HTTP Port:", WS_CHILD | WS_VISIBLE, 20, y+2, lblW, h, hWnd, NULL, g_hInstance, NULL);
            CreateWindow("EDIT", buf, WS_CHILD | WS_VISIBLE | WS_BORDER | ES_NUMBER, 135, y, editW, h, hWnd, (HMENU)ID_EDIT_HTTP, g_hInstance, NULL);
            y += spacing;

            snprintf(buf, sizeof(buf), "%d", g_DicomPort);
            CreateWindow("STATIC", "DICOM Port:", WS_CHILD | WS_VISIBLE, 20, y+2, lblW, h, hWnd, NULL, g_hInstance, NULL);
            CreateWindow("EDIT", buf, WS_CHILD | WS_VISIBLE | WS_BORDER | ES_NUMBER, 135, y, editW, h, hWnd, (HMENU)ID_EDIT_DICOM, g_hInstance, NULL);
            y += spacing;

            snprintf(buf, sizeof(buf), "%d", g_TelnetTimeout);
            CreateWindow("STATIC", "Timeout (sec):", WS_CHILD | WS_VISIBLE, 20, y+2, lblW, h, hWnd, NULL, g_hInstance, NULL);
            CreateWindow("EDIT", buf, WS_CHILD | WS_VISIBLE | WS_BORDER | ES_NUMBER, 135, y, editW, h, hWnd, (HMENU)ID_EDIT_TIMEOUT, g_hInstance, NULL);
            y += spacing;

            snprintf(buf, sizeof(buf), "%d", g_DebugLog);
            CreateWindow("STATIC", "Debug Log (0/1):", WS_CHILD | WS_VISIBLE, 20, y+2, lblW, h, hWnd, NULL, g_hInstance, NULL);
            CreateWindow("EDIT", buf, WS_CHILD | WS_VISIBLE | WS_BORDER | ES_NUMBER, 135, y, editW, h, hWnd, (HMENU)ID_EDIT_DEBUG, g_hInstance, NULL);
            y += spacing + 15;

            CreateWindow("BUTTON", "Save Settings", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 20, y, 150, 32, hWnd, (HMENU)ID_BTN_SAVE, g_hInstance, NULL);
            CreateWindow("BUTTON", "", WS_CHILD | WS_VISIBLE | BS_OWNERDRAW, 195, y, 150, 32, hWnd, (HMENU)ID_BTN_TOGGLE, g_hInstance, NULL);
            return 0;
        }
        case WM_COMMAND: {
            ...
            if (cmd == ID_BTN_SAVE) {
                char val[64];
                GetDlgItemText(hWnd, ID_EDIT_AET, val, sizeof(val));
                WritePrivateProfileString(szIniServer, szIniAET, val, szIniFile);
                SetAETitle(val);

                GetDlgItemText(hWnd, ID_EDIT_TELNET, val, sizeof(val));
                WritePrivateProfileString(szIniServer, szIniTelnetPort, val, szIniFile);
                g_TelnetPort = atoi(val);

                GetDlgItemText(hWnd, ID_EDIT_HTTP, val, sizeof(val));
                WritePrivateProfileString(szIniServer, szIniHttpPort, val, szIniFile);
                g_HttpPort = atoi(val);

                GetDlgItemText(hWnd, ID_EDIT_DICOM, val, sizeof(val));
                WritePrivateProfileString(szIniServer, szIniDicomPort, val, szIniFile);
                g_DicomPort = atoi(val);

                GetDlgItemText(hWnd, ID_EDIT_TIMEOUT, val, sizeof(val));
                WritePrivateProfileString(szIniServer, szIniTelnetTimeout, val, szIniFile);
                g_TelnetTimeout = atoi(val);

                GetDlgItemText(hWnd, ID_EDIT_DEBUG, val, sizeof(val));
                WritePrivateProfileString(szIniServer, szIniDebugLog, val, szIniFile);
                g_DebugLog = atoi(val);

                MessageBox(hWnd, "Settings saved to INI file!\n(Note: Restart servers if port numbers were modified)", "Settings Saved", MB_OK | MB_ICONINFORMATION);
                return 0;
            }
            if (cmd == ID_BTN_TOGGLE) {
                if (g_bRunning) ServerStop(); else ServerStart();
                InvalidateRect(GetDlgItem(hWnd, ID_BTN_TOGGLE), NULL, TRUE);
                return 0;
            }
            break;
        }
        case WM_DRAWITEM: {
            ...
            if (lpDIS->CtlID == ID_BTN_TOGGLE) {
                ...
                if (g_bRunning) {
                    hBrush = CreateSolidBrush(RGB(76, 175, 80)); /* Green background when running */
                    textColor = RGB(255, 255, 255);
                    btnText = "Stop Server (RUNNING)";
                } else {
                    hBrush = CreateSolidBrush(RGB(220, 220, 220)); /* Standard grey when stopped */
                    textColor = RGB(0, 0, 0);
                    btnText = "Start Server (STOPPED)";
                }
                FillRect(hdc, &rc, hBrush);
                DeleteObject(hBrush);

                DrawEdge(hdc, &rc, (lpDIS->itemState & ODS_SELECTED) ? EDGE_SUNKEN : EDGE_RAISED, BF_RECT);
                
                int oldBkMode = SetBkMode(hdc, TRANSPARENT);
                COLORREF oldTextCol = SetTextColor(hdc, textColor);
...
static void OpenSettingsWindow(void) {
    ...
    if (!classRegistered) {
        ...
        wc.lpszClassName = "RisSettingsWndClass";
        RegisterClassEx(&wc);
        classRegistered = 1;
    }
    
    g_hSettingsWnd = CreateWindowEx(WS_EX_DLGMODALFRAME, "RisSettingsWndClass", "RIS Server Settings",
...
static LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    if (uMsg == WM_COMMAND) {
        ...
        if (cmd == ID_TRAY_SETTINGS) { OpenSettingsWindow(); return 0; }
