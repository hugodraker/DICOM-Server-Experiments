...
szHtmlStart        db "<html><head><title>RIS Worklist Manager</title></head><body style='font-family: Arial; padding: 20px;'>",0
szHtmlHeader       db "<h1>RIS Worklist Manager</h1>",0
szHtmlForms        db "<h3>Upload patients.csv</h3>"
                   db "<form action='/upload' method='POST' enctype='multipart/form-data'><input type='file' name='csvfile'><input type='submit' value='Upload CSV'></form>"
                   ...
                   db "<input type='text' name='PatientName' placeholder='Patient Name'> <input type='text' name='PatientID' placeholder='Patient ID'><br><br>"
                   db "<input type='text' name='Accession' placeholder='Accession Number'> <input type='text' name='Modality' placeholder='Modality'><br><br>"
...
ProcessHttpClient PROC clientIdx:DWORD, pBuf:DWORD
    LOCAL sock:DWORD
    LOCAL hFile:DWORD
...
ServeHttpUI:
    ...
    ; Dump current CSV to HTML table
    invoke crt_fopen, OFFSET szCSVFile, OFFSET szModeRead
    mov hFile, eax
    cmp hFile, 0
    je ServeEnd
    
    invoke SendText, sock, CStr("<table border='1'>")
CSVLoop:
    invoke crt_fgets, OFFSET g_fileLine, LINE_SIZE, hFile
    cmp eax, 0
    je CloseHTML
    
    invoke SendText, sock, CStr("<tr><td>")
    invoke SendText, sock, OFFSET g_fileLine
    invoke SendText, sock, CStr("</td><td><button>Edit</button></td></tr>")
    jmp CSVLoop
...
HandleHttpPost:
    ; -------------------------------------------------------------
    ; [POST PARSING LOGIC HERE]
    ; To complete this without a framework, you must:
    ; 1. Search `pBuf` for the multipart boundary string.
    ; 2. Extract the payload and call `UpdateCsvByPatient` loop.
    ; 3. Once files are parsed/written, return a 302 Redirect to `/`.
    ; -------------------------------------------------------------
    invoke SendText, sock, OFFSET szHttp200OK
    invoke SendText, sock, OFFSET szHtmlStart
    invoke SendText, sock, CStr("<h2>Action Processed.</h2><a href='/'>Return</a>")
    invoke SendText, sock, OFFSET szHtmlEnd
    invoke RemoveClient, clientIdx
    ret
...
ARB_NextInc:
    ...
    cmp isHttp, 1
    jne ARB_Loop
    ; In a real implementation you check for \r\n\r\n here to flush ProcessHttpClient
    jmp ARB_Loop
...
ARB_Done:
    ...
    ; Route HTTP processing once payload finishes
    cmp isHttp, 1
    jne ARB_Ret
