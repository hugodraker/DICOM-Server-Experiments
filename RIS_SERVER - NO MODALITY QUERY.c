...
static void handle_pdata_tf(SOCKET clientSocket, char* buffer, int bytesRead) {
    ...
    for(int i = 12; i < bytesRead - 10; i++) {
        /* Command Field (0000,0100) — Implicit VR, VL=2, value is US */
        if(buffer[i]==0x00 && buffer[i+1]==0x00 &&
           buffer[i+2]==0x00 && buffer[i+3]==0x01 &&
           buffer[i+4]==0x02 && buffer[i+5]==0x00)
            command = ((uint16_t)buffer[i+9] << 8) | (uint16_t)buffer[i+8];

        /* Message ID (0000,0120) — Implicit VR, VL=2, value is US */
        if(buffer[i]==0x00 && buffer[i+1]==0x00 &&
           buffer[i+2]==0x20 && buffer[i+3]==0x01 &&
           buffer[i+4]==0x02 && buffer[i+5]==0x00)
            msg_id = ((uint16_t)buffer[i+9] << 8) | (uint16_t)buffer[i+8];
    }
    if (command == 0x0030) handle_c_echo_rq(clientSocket, pc_id, msg_id);
    else if (command == 0x0020) handle_c_find_rq(clientSocket, pc_id, msg_id, (uint8_t*)buffer, bytesRead);
}
