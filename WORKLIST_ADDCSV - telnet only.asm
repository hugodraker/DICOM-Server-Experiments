...
szTrayTip          db "RIS Telnet Server",0
szMenuStart        db "Start Server",0
szMenuStop         db "Stop Server",0
szMenuShow         db "Show Console",0
szMenuExit         db "Exit",0

szStartup          db "RIS Telnet Server starting...",13,10,0
szConfigFmt        db "Config: AET=%s TelnetPort=%u TelnetTimeout=%u DebugLog=%u",13,10,0
...
IsFourDigits PROC uses esi pStr:DWORD
    LOCAL idx:DWORD
    mov esi, pStr
    mov idx, 0
    movzx eax, BYTE PTR [esi]
...
; NEW FUNCTION: Compare Accession Number (field 8) between CSV line and entry
AccessionEquals PROC uses esi edi pLine:DWORD, pAccession:DWORD
    LOCAL tempEntry[ENTRY_SIZE]:BYTE
    LOCAL pField:DWORD
    ; Parse the CSV line into a temp entry
    invoke ParseCSVLineToEntry, pLine, ADDR tempEntry
    ; Get pointer to field 8 (Accession Number)
    invoke GetFieldPtr, ADDR tempEntry, 8
    mov pField, eax
    ; Compare with provided accession
...
UpdateCsvByPatient PROC pEntry:DWORD, pLineOut:DWORD
    ...
    LOCAL hOut:DWORD
    LOCAL found:DWORD
    LOCAL pAccession:DWORD      ; Changed from pPatientID
    mov found, 0
    invoke CreateCSVIfMissing
    invoke GetFieldPtr, pEntry, 8     ; Get field 8 (Accession) instead of field 1
    mov pAccession, eax
    invoke crt_fopen, OFFSET szCSVFile, OFFSET szModeRead
    mov hIn, eax
...
UCP_ReadLoop:
    ...
    cmp found, 1
    je UCP_CopyOriginal
    invoke AccessionEquals, OFFSET g_fileLine, pAccession   ; Changed from FirstFieldEquals
    cmp eax, 1
...
CommitPendingEntry PROC clientIdx:DWORD
    LOCAL pEntry:DWORD
    LOCAL pAccession:DWORD      ; Renamed from pPatientID
    LOCAL sock:DWORD
    LOCAL found:DWORD
    ...
    mov found, eax
    invoke GetFieldPtr, pEntry, 8   ; Changed from field 1 to field 8
    mov pAccession, eax              ; Changed variable name
    invoke GetClientSockPtr, clientIdx
    mov eax, DWORD PTR [eax]
    mov sock, eax
    cmp found, 1
...
PCL2_Valid:
    ...
    invoke GetFieldPtr, pPending, 8   ; Changed from field 1 to field 8
    mov pAccession, eax               ; Changed variable name
...
Main PROC
    ...
    invoke LoadConfig
    invoke crt_printf, OFFSET szConfigFmt, OFFSET g_aeCalled, g_TelnetPort, g_TelnetTimeout, g_DebugLog
    invoke InitArrays
    invoke InitSockets
    invoke CreateCSVIfMissing
    invoke CreateTray
